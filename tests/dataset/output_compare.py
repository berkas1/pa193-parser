...
def check_toc(actual, expected):
    if not "table_of_content" in actual:
        if not "table_of_content" in expected:
            return 20
        return 0

    actual = list(filter(lambda x: len(x) == 3, actual["table_of_content"]))
    expected = expected["table_of_content"]
